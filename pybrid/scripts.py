# setup function prepares the experiment (configuration, MNIST datasets, dataloaders, model, and optimizer). 
# The main function then iterates over mini-batches, calling model.train_batch(...), performing optimizer updates, and accumulating losses, errors, and inference iteration counts. 
# After every batches_per_epoch training batches, it aggregates metrics, runs evaluation on the test set for hybrid PC, pure PC, and the amortised network, 
# generates sample reconstructions, and saves all metrics to metrics.json. 
# This script is mainly used to produce the training curves and evaluation statistics reported in the experiments.


#nastavíš, koľko batchov chceš spracovať celkovo, môžeš ľahšie porovnávať experimenty pri rovnakom počte batchov, aj keď sa mení batch_size alebo batches_per_epoch.
#alternativa k _scripts ale trosku tazky lebo sa da kontorlovat trening podla poctu batchov
...
def setup(cfg):
    ...
    #NEW
    train_dataset = datasets.SharedMNIST( train=True, 
                                         normalize=cfg.data.normalize, 
                                         flatten=True, 
                                         return_one_hot=True,          
                                        scale=cfg.data.label_scale )
    
    test_dataset = datasets.SharedMNIST( train=False,
                                        normalize=cfg.data.normalize,
                                        flatten=True,
                                          return_one_hot=True,
                                          scale=cfg.data.label_scale
        ...
        while global_batch_id < cfg.exp.num_batches:
            for batch_id, (img_batch, label_batch) in enumerate(train_loader):
                ...
                if (global_batch_id % cfg.exp.batches_per_epoch == 0) and global_batch_id > 0:
                    ...
                    ##NEW
                    model.save_checkpoint(cfg.exp.log_dir + f"/checkpoints/epoch{curr_epoch}.pth", extra={"epoch": curr_epoch, "global_batch_id": global_batch_id ,"seed": cfg.exp.seed, "config_path": cfg.exp.log_dir + "/config.json",})
