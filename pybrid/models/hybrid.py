...
class HybridModel(BaseModel):
    ...
    def save_checkpoint(self, path, extra=None):
        ckpt = {
            "extra": extra or {},
            "nodes": self.nodes,
            "amort_nodes": self.amort_nodes,
            "mu_dt": self.mu_dt,
            "pc": [  {
                    "W": l.weights.detach().cpu(),
                    "b": l.bias.detach().cpu(),
                }
            ...
            "amort": [ {
                ...
                for l in self.amort_layers ],
        }
        torch.save(ckpt, path)


    def load_checkpoint(self, path, device=None, map_location="cpu"):
        device = device or utils.get_device()
        ckpt = torch.load(path, map_location=map_location)

        for l, sd in zip(self.layers, ckpt["pc"]):
            l.weights = sd["W"].to(device).float()
            l.bias = sd["b"].to(device).float()

        for l, sd in zip(self.amort_layers, ckpt["amort"]):
            l.weights = sd["W"].to(device).float()
            l.bias = sd["b"].to(device).float()
